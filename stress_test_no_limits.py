#!/usr/bin/env python3
"""
HustleClub App Stress Test - No Client-Side Limits
==================================================

This version bypasses client-side rate limiting to get accurate token usage data.
It directly calls the API without going through the frontend rate limiting.

Usage:
    python3 stress_test_no_limits.py
    
To test with different parameters:
    python3 stress_test_no_limits.py --users 20 --queries 10 --delay 1.0
"""
...
class StressTest:
    ...
    def run_test(self):
        ...
        print(f"\n{'='*60}")
        print(f"HUSTLECLUB APP STRESS TEST (No Client-Side Limits)")
        print(f"{'='*60}")
    ...
    def generate_report(self):
        ...
        if self.success_count > 0:
            avg_tokens = (self.total_tokens['input'] + self.total_tokens['output']) / self.success_count
        ...
        print(f"\nCOST ANALYSIS (Mistral Pricing - $0.25/1M tokens):")
        print(f"Estimated Input Cost: ${input_cost:.6f}")
        print(f"Estimated Output Cost: ${output_cost:.6f}")
        print(f"Total Estimated Cost: ${total_cost:.6f}")
        if self.num_users > 0:
            print(f"Cost per User: ${total_cost / self.num_users:.6f}")
        if self.success_count > 0:
            print(f"Cost per Request: ${total_cost / self.success_count:.8f}")
        
        # Token Cost Analysis
        if self.success_count > 0:
            avg_input = self.total_tokens['input'] / self.success_count
            avg_output = self.total_tokens['output'] / self.success_count
            avg_cost = (avg_input + avg_output) / 1_000_000 * 0.25
            print(f"\nTOKEN COST ANALYSIS:")
            print(f"Average Input Tokens per Request: {avg_input:.1f}")
            print(f"Average Output Tokens per Request: {avg_output:.1f}")
            print(f"Average Cost per Request: ${avg_cost:.8f}")
        ...
        if self.success_count > 0:
            ...
            response_count = 0
            for user_id in self.user_tokens:
                for result in self.user_tokens[user_id].get('results', []):
                    if isinstance(result, dict) and result.get('success') and 'response_time' in result:
            ...
            if duration > 0:
                print(f"Requests per Second: {self.success_count / duration:.2f}")
        
        # Extrapolation: Cost for 100 users, 1000 users
        if self.success_count > 0:
            cost_per_request = total_cost / self.success_count
            print(f"\nEXTRAPOLATION:")
            print(f"Estimated cost for 100 users (10 queries each): ${cost_per_request * 1000:.6f}")
            print(f"Estimated cost for 1000 users (10 queries each): ${cost_per_request * 10000:.6f}")
    ...
    def save_report_to_file(self):
        ...
        with open(self.output_file, 'w') as f:
            f.write(f"HUSTLECLUB APP STRESS TEST REPORT (No Client-Side Limits)\n")
            ...
            if self.success_count > 0:
                avg_tokens = (self.total_tokens['input'] + self.total_tokens['output']) / self.success_count
                f.write(f"Average Tokens per Request: {avg_tokens:.1f}\n\n")
            ...
            f.write(f"Cost per User: ${total_cost / self.num_users if self.num_users > 0 else 0:.6f}\n")
            f.write(f"Cost per Request: ${total_cost / self.success_count if self.success_count > 0 else 0:.8f}\n\n")
...
def main():
    parser = argparse.ArgumentParser(description='HustleClub App Stress Test - No Client-Side Limits')
    ...
    try:
        response = requests.get(f"{args.api_url.rstrip('/api/chat')}", timeout=5)
        if response.status_code != 200:
